...
def Get_JulianDate_From_Arrayed_yyyyMMdd2(date = ["2011","1","21"], ssssss = 0.0):
    ...
    if 1 <= month <= 2:
        ...
        raise ValueError, u"月の指定に誤りがあります。0以下であったり13以上になっていないか確認してください。"
    else:
        y = year
        m = month
...
def Get_DayOfWeek_From_DelimitedDate(date = "2010.3.30"):
    ...
    if len(_date_arr) == 3:
        ...
        raise ValueError, u"引数の解析エラーが発生しました。引数で渡された日付はyyyy/MM/ddまたはyyyy.MM.dd形式となっていますか？"
...
def Get_ddmmyy_From_Delimited_yyyyMMdd(date = "2010/05/30"):
    """ "yyyy/mm/dd"形式の日付をNMEAのddmmyy形式へ変換する.
...
def main():
    print(u"セルフテスト& サンプルコード（コードを読んで下さい）")
    print(u"うるう年判定 " + u"2000年: " + str(IsLeapYear(2000)) + u", 2001年: " + str(IsLeapYear(2001)) + u", 2004年: " + str(IsLeapYear(2004)) + u", 2100年: " + str(IsLeapYear(2100)))
    print(u"月末日 " + u"2000年2月: " + str(Get_LastDayOfMonth(2000, 2)) + u", 2001年2月: " + str(Get_LastDayOfMonth(2001, 2)))
    print(u"GPS日の演算テスト GPS元期 " + str(GPSepoch_JD) + ": " + str(Get_GPSday_From_JulianDate(GPSepoch_JD)) + u", 2000年1月1日: " + str(Get_GPSday_From_JulianDate(Get_JulianDate_From_Arrayed_yyyyMMdd2([2000, 1, 1]))))
    print(u"ユリウス日からGPS週番号を求めるテスト 1999年8月22日: " + str(Get_GPSweek_From_JulianDate(Get_JulianDate_From_Arrayed_yyyyMMdd2([1999, 8, 22]))))
    print(u"修正ユリウス日のテスト　GPS元期 " + str(GPSepoch_JD) + ": " + str(Get_ModifiedJulianDate_From_JuliusDate(GPSepoch_JD)))
    print(u"ユリウス日のテスト　その1　リストを利用しています。 [2000, 1, 1](2000年1月1日): " + str(Get_JulianDate_From_Arrayed_yyyyMMdd([2000, 1, 1])))
    print(u"ユリウス日のテスト　その2  リストを利用しています。 [2000, 1, 1](2000年1月1日): " + str(Get_JulianDate_From_Arrayed_yyyyMMdd2([2000, 1, 1])))
    print(u"ユリウス日のテスト　その3↓\n演算アルゴリズムの異なる処理を2通り実行していますので比較してみてください。")
    ...
    print(u"ユリウス日のテスト　その4 　デリミタの無い日付yyyyMMddを使って計算しています。 20000101（2000年1月1日）: " + str(Get_JulianDate_From_yyyyMMdd(20000101)))
    print(u"曜日番号のテスト \"2000/1/1\": " + str(Get_DayOfWeek_From_DelimitedDate("2000/1/1")))
    print(u"年間の通算日のテスト \"2000/1/1\": " + str(Get_DayOfYear_From_DelimitedDate("2000/1/1")))
    print(u"日付文字列からGPS週番号を求めるテスト \"2000/1/1\": " + str(Get_GPSweek_From_DelimitedDate("2000/1/1")))
    print(u"日付から曜日を取得 2012年1月13日: " + listOfWeekName[Get_DayOfWeek_From_yyyyMMdd(20120113)])
    print(u"時分秒を指定して、通算秒を得るテスト 000130（0h1m30s）: " + str(Get_ssssss_From_hhmmss("000130")) + u" s")
    print(u"時刻から通算秒を求めるテスト 134059（13h40m59s）: " + str(Get_ssssss_From_hhmmss(134059)))
    print(u"デリミタのある時刻から通算秒を求めるテスト その1 \"13:40:50\" :  " +  str(Get_ssssss_From_Delimited_hhmmss("13:40:59")))
    print(u"デリミタのある時刻から通算秒を求めるテスト その2 \"13:40.50\" :  " +  str(Get_ssssss_From_Delimited_hhmmss2("13:40.59")))
    print(u"通算秒からデリミタのある時刻文字列を求めるテスト 49259:  " + str(Get_Delimited_hhmmss_From_ssssss(49259)))
    print(U"通算秒からデリミタのない時刻を求めるテスト 49259:  " + str(Get_hhmmss_From_ssssss(49259)))
    print(u"通算秒表現の日本標準時（JST）をUTCへ変換するテスト49259: " + str(Get_JST_From_UTC(49259)) + u" 見やすくするためにデリミタ付き時分秒: " + str(Get_Delimited_hhmmss_From_ssssss(Get_JST_From_UTC(49259))))
    print(u"デリミタの無い日付yyyyMMddをNMEAで使用されているddMMyyへ変換するテスト 20120101（2012年1月1日）: " + str(Get_ddMMyy_From_yyyyMMdd(20120101)))
    print(u"デリミタの有る日付\"yyyy/MM/dd\"をNMEAで使用されているddMMyyへ変換するテスト \"2012/1/1\": " + str(Get_ddmmyy_From_Delimited_yyyyMMdd("2012/1/1")))
    print(u" \"MM/dd/yyyy\"形式の日付を\"yyyy/MM/dd\"形式へ変換するテスト \"5/25/2012\": " + str(Get_Delimited_yyyyMMdd_From_Delimited_MMddyyyy("5/25/2012")))
    print(u" \"dd/MM/yyyy\"形式の日付を\"yyyy/MM/dd\"形式へ変換するテスト \"25/5/2012\": " + str(Get_Delimited_yyyyMMdd_From_Delimited_ddMMyyyy("25/5/2012")))
    print(u"デリミタの無い日付と通算秒からMicrosoft Office Excel時刻を計算するテスト 19000101（1900年1月1日）: " + str(Get_ExcelTime_From_yyyyMMdd_and_ssssss(19000101)) + u", 20000101（2000年1月1日）: " + str(Get_ExcelTime_From_yyyyMMdd_and_ssssss(20000101)))
