...
# Licence:     MIT
# Histroy:
#           2014/3/1     GPS用に作成したモジュールをほぼコピー。
#                        readerクラスの初期化部分において、放送歴の拡張子部分だけ変更した。
#                        [課題] 衛星名とPRN番号の割り当てを変える必要がある・・・かも。
#                               ただし、衛星入れ替わりとともに衛星名とPRNの組み合わせが変わるようだと使いにくい。
#-------------------------------------------------------------------------------
...
import datetime
import timeKM
import gnss.qzs.time as qtime
import gnss.datum.GRS80 as grs80
import gnss.qzs.coordinate as qcoor
...
class reader(gnss_eph.reader):
    ...
    def __init__(self):
        """
        """
        gnss_eph.reader.__init__(self, "\.\d{2}[q]", eph_pattern, ephemeris) 
...
def main():
    ...
    # エフェメリスを読み込むテスト
    test_str = \
        "J 1 10  9 10  0  0  0.0-0.160860363394D-03-0.386535248253D-11 0.000000000000D+00\n" + \
        ...
        "J 2 10  9 10  0  0  0.0-0.160860363394D-03-0.386535248253D-11 0.000000000000D+00\n" + \
        "    0.690000000000D+02 0.305937500000D+02 0.490449000598D-08-0.641460797771D+00\n" + \
        "    0.176765024662D-05 0.449971400667D-02 0.395439565182D-05 0.515361504936D+04\n" + \
        "    0.432000000000D+06 0.558793544769D-08 0.167197328015D+01 0.409781932831D-07\n" + \
        "    0.965529665683D+00 0.305468750000D+03 0.956638718151D+00-0.849392523465D-08\n" + \
        ...
        "    0.200000000000D+01 0.630000000000D+02-0.190921127796D-07 0.690000000000D+02\n" + \
        "    0.424800000000D+06 0.000000000000D+00 0.000000000000D+00 0.000000000000D+00\n" + \
        "J 3 10  9 10  0  0  0.0-0.160860363394D-03-0.386535248253D-11 0.000000000000D+00\n" + \
        "    0.690000000000D+02 0.305937500000D+02 0.490449000598D-08-0.641460797771D+00\n" + \
        "    0.176765024662D-05 0.449971400667D-02 0.395439565182D-05 0.515361504936D+04\n" + \
        "    0.432000000000D+06 0.558793544769D-08 0.167197328015D+01 0.409781932831D-07\n" + \
        "    0.965529665683D+00 0.305468750000D+03 0.956638718151D+00-0.849392523465D-08\n" + \
    ...
    # エフェメリスの読み込みテスト
    ephs = r.read_ephemeris("brdc0010.13q")
    ...
    def create_filter(epoch):
        ...
        def filter(system_name, ephs_dict):
            ...
            return ephs
        return filter
    epoch = 1721 * timeKM.TIME_A_WEEK + 86400 * 2 + 3600 * 7
    print("\n")
