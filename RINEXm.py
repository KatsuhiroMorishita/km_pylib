...
# Licence:     new BSD
# History:     2013/1/3  isRINEX()の判定において、OとNも認識するように変更した。
...
class Epoch:
    """ エポックを表すクラス
    """
    def __init__(self, timeSystemName = "GPS", time = GPS.epochOrigin):
        """
        コンストラクタ
        Args:
    ...
    def pickOut(self, t1, t2):
        ...
        if self.isSet:
            if (isinstance(t1, datetime.datetime) and isinstance(t2, datetime.datetime)):
                if(t1 < t2):
                    first = GPS.epochOrigin
                    last = GPS.epochOrigin
                    _copyEnable = False
                    __txt = []
                    for line in self._txt:
                        _epoch = getDateFromRinexBodyEpoch(line)# 文字列解析してエポックを取得
